...
class SetupChecks {
    static validate(rushConfiguration) {
        ...
        if (errorMessage) {
            ...
            console.error(terminal_1.Colorize.red(terminal_1.PrintUtilities.wrapWords(errorMessage)));
    ...
    static _validate(rushConfiguration) {
        ...
        if (rushConfiguration.packageManager === 'pnpm') {
            if (semver.lt(rushConfiguration.packageManagerToolVersion, MINIMUM_SUPPORTED_PNPM_VERSION)) {
                return (`The ${RushConstants_1.RushConstants.rushJsonFilename} file requests PNPM version ` +
            ...
            if (semver.lt(rushConfiguration.packageManagerToolVersion, MINIMUM_SUPPORTED_NPM_VERSION)) {
                return (`The ${RushConstants_1.RushConstants.rushJsonFilename} file requests NPM version ` +
                    rushConfiguration.packageManagerToolVersion +
    ...
    static _checkForPhantomFolders(rushConfiguration) {
        ...
        if (phantomFolders.length > 0) {
            if (phantomFolders.length === 1) {
                // eslint-disable-next-line no-console
                console.log(terminal_1.Colorize.yellow(terminal_1.PrintUtilities.wrapWords('Warning: A phantom "node_modules" folder was found. This defeats Rush\'s protection against' +
                    ' NPM phantom dependencies and may cause confusing build errors. It is recommended to' +
                    ' delete this folder:')));
            }
            else {
                // eslint-disable-next-line no-console
                console.log(terminal_1.Colorize.yellow(terminal_1.PrintUtilities.wrapWords('Warning: Phantom "node_modules" folders were found. This defeats Rush\'s protection against' +
                    ' NPM phantom dependencies and may cause confusing build errors. It is recommended to' +
                    ' delete these folders:')));
            }
            for (const folder of phantomFolders) {
                // eslint-disable-next-line no-console
                console.log(terminal_1.Colorize.yellow(`"${folder}"`));
            }
            // eslint-disable-next-line no-console
