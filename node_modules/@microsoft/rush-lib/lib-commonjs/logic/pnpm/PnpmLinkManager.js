...
class PnpmLinkManager extends BaseLinkManager_1.BaseLinkManager {
    ...
    async createSymlinksForProjectsAsync(force) {
        ...
        if (useWorkspaces) {
            ...
            console.log(terminal_1.Colorize.red('Linking is not supported when using workspaces. Run "rush install" or "rush update" ' +
    ...
    async _linkProjectsAsync() {
        if (this._rushConfiguration.projects.length > 0) {
            ...
            const pnpmShrinkwrapFile = PnpmShrinkwrapFile_1.PnpmShrinkwrapFile.loadFromFile(this._rushConfiguration.defaultSubspace.getTempShrinkwrapFilename());
            if (!pnpmShrinkwrapFile) {
                throw new node_core_library_1.InternalError(`Cannot load shrinkwrap at "${this._rushConfiguration.defaultSubspace.getTempShrinkwrapFilename()}"`);
            }
            ...
            // eslint-disable-next-line no-console
            console.log(terminal_1.Colorize.yellow(`\nWarning: Nothing to do. Please edit ${RushConstants_1.RushConstants.rushJsonFilename} and add at least one project` +
    ...
    async _linkProjectAsync(project, pnpmShrinkwrapFile) {
        ...
        await (0, link_bins_1.default)(projectFolder, projectBinFolder, {
            warn: (msg) => {
                // eslint-disable-next-line no-console
                console.warn(terminal_1.Colorize.yellow(msg));
            }
        });
    }
    async _getPathToLocalInstallationAsync(tarballEntry, absolutePathToTgzFile, folderSuffix) {
        if (this._pnpmVersion.major === 6) {
            ...
            const { depPathToFilename } = await Promise.resolve().then(() => __importStar(require('@pnpm/dependency-path')));
            ...
            const { depPathToFilename } = await Promise.resolve().then(() => __importStar(require('dependency-path')));
            ...
            const folderNameInLocalInstallationRoot = (0, strict_uri_encode_1.default)(node_core_library_1.Path.convertToSlashes(absolutePathToTgzFile)) + folderSuffix;
    ...
    _createLocalPackageForDependency(project, parentShrinkwrapEntry, localPackage, pathToLocalInstallation, dependencyName, isOptional = false) {
        ...
        const newLocalFolderPath = path.join(localPackage.folderPath, 'node_modules', dependencyName);
        const version = (0, PnpmShrinkwrapFile_1.normalizePnpmVersionSpecifier)(versionSpecifier);
