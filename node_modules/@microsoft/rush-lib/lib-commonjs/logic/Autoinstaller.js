...
class Autoinstaller {
    constructor(options) {
        ...
        this._restrictConsoleOutput =
            (_a = options.restrictConsoleOutput) !== null && _a !== void 0 ? _a : RushCommandLineParser_1.RushCommandLineParser.shouldRestrictConsoleOutput();
    ...
    static validateName(autoinstallerName) {
        const nameOrError = node_core_library_1.PackageName.tryParse(autoinstallerName);
    ...
    async prepareAsync() {
        ...
        await InstallHelpers_1.InstallHelpers.ensureLocalPackageManagerAsync(this._rushConfiguration, this._rushGlobalFolder, RushConstants_1.RushConstants.defaultMaxInstallAttempts, this._restrictConsoleOutput);
        // Example: common/autoinstallers/my-task/package.json
        ...
        const lock = await node_core_library_1.LockFile.acquireAsync(autoinstallerFullPath, 'autoinstaller');
        try {
            ...
            const nodeModulesFolder = `${autoinstallerFullPath}/${RushConstants_1.RushConstants.nodeModulesFolderName}`;
            const flagPath = `${nodeModulesFolder}/rush-autoinstaller.flag`;
            const isLastInstallFlagDirty = !(await lastInstallFlag.isValidAsync()) || !node_core_library_1.FileSystem.exists(flagPath);
            if (isLastInstallFlagDirty || lock.dirtyWhenAcquired) {
                ...
                Utilities_1.Utilities.syncNpmrc({
                ...
                node_core_library_1.FileSystem.writeFile(flagPath, 'If this file is deleted, Rush will assume that the node_modules folder has been cleaned and will reinstall it.');
    ...
    async updateAsync() {
        ...
        if (!(await node_core_library_1.FileSystem.existsAsync(autoinstallerPackageJsonPath))) {
        ...
        let oldFileContents = '';
        if (await node_core_library_1.FileSystem.existsAsync(this.shrinkwrapFilePath)) {
            ...
            await node_core_library_1.FileSystem.deleteFileAsync(this.shrinkwrapFilePath);
            if (this._rushConfiguration.packageManager === 'pnpm') {
                ...
                await node_core_library_1.FileSystem.deleteFileAsync(path.join(this.folderFullPath, pnpmPackageManager.internalShrinkwrapRelativePath));
            }
        }
        ...
        if (this._rushConfiguration.packageManager === 'npm') {
            this._logIfConsoleOutputIsNotRestricted(terminal_1.Colorize.bold('Running "npm shrinkwrap"...'));
            await Utilities_1.Utilities.executeCommandAsync({
                command: this._rushConfiguration.packageManagerToolFilename,
                args: ['shrinkwrap'],
                workingDirectory: this.folderFullPath,
        ...
        const newFileContents = await node_core_library_1.FileSystem.readFileAsync(this.shrinkwrapFilePath, {
            convertLineEndings: node_core_library_1.NewlineKind.Lf
        });
        if (oldFileContents !== newFileContents) {
            this._logIfConsoleOutputIsNotRestricted(terminal_1.Colorize.green('The shrinkwrap file has been updated.') + '  Please commit the updated file:');
            this._logIfConsoleOutputIsNotRestricted(`\n  ${this.shrinkwrapFilePath}`);
        }
        else {
            this._logIfConsoleOutputIsNotRestricted(terminal_1.Colorize.green('Already up to date.'));
        }
    }
