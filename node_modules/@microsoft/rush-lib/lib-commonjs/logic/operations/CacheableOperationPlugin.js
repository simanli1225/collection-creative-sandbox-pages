...
class CacheableOperationPlugin {
    ...
    apply(hooks) {
        ...
        hooks.beforeExecuteOperations.tap(PLUGIN_NAME, (recordByOperation, context) => {
            ...
            const definitelyDefinedInputsSnapshot = inputsSnapshot;
            const disjointSet = (cobuildConfiguration === null || cobuildConfiguration === void 0 ? void 0 : cobuildConfiguration.cobuildFeatureEnabled)
                ...
                : undefined;
            const hashByOperation = new Map();
            // Build cache hashes are computed up front to ensure stability and to catch configuration errors early.
            function getOrCreateOperationHash(operation) {
                var _a;
                const cachedHash = hashByOperation.get(operation);
                if (cachedHash !== undefined) {
                    return cachedHash;
                }
                // Examples of data in the config hash:
                // - CLI parameters (ShellOperationRunner)
                const configHash = (_a = operation.runner) === null || _a === void 0 ? void 0 : _a.getConfigHash();
                ...
                const localStateHash = associatedProject &&
                    definitelyDefinedInputsSnapshot.getOperationOwnStateHash(associatedProject, associatedPhase === null || associatedPhase === void 0 ? void 0 : associatedPhase.name);
                ...
                const dependencyHashes = Array.from(operation.dependencies, getDependencyHash).sort();
                const hasher = crypto.createHash('sha1');
                // This property is used to force cache bust when version changes, e.g. when fixing bugs in the content
                // of the build cache.
                hasher.update(`${RushConstants_1.RushConstants.buildCacheVersion}`);
                ...
                if (localStateHash) {
                    hasher.update(`${RushConstants_1.RushConstants.hashDelimiter}${localStateHash}`);
                }
                if (configHash) {
                    hasher.update(`${RushConstants_1.RushConstants.hashDelimiter}${configHash}`);
                }
                const hashString = hasher.digest('hex');
                hashByOperation.set(operation, hashString);
                return hashString;
            }
            function getDependencyHash(operation) {
                return `${RushConstants_1.RushConstants.hashDelimiter}${operation.name}=${getOrCreateOperationHash(operation)}`;
            }
            for (const [operation, record] of recordByOperation) {
                ...
                const stateHash = getOrCreateOperationHash(operation);
    ...
    _tryGetProjectBuildCache({ buildCacheConfiguration, buildCacheContext, rushProject, phase, terminal, operation }) {
        var _a;
        if (!buildCacheContext.operationBuildCache) {
            ...
            buildCacheContext.operationBuildCache = ProjectBuildCache_1.ProjectBuildCache.getProjectBuildCache({
                project: rushProject,
