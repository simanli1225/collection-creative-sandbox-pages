...
class ShrinkwrapFileFactory {
    static getShrinkwrapFile(packageManager, packageManagerOptions, shrinkwrapFilename) {
        switch (packageManager) {
            ...
            case 'pnpm':
                return PnpmShrinkwrapFile_1.PnpmShrinkwrapFile.loadFromFile(shrinkwrapFilename);
            case 'yarn':
    ...
    static parseShrinkwrapFile(packageManager, packageManagerOptions, shrinkwrapContent) {
        switch (packageManager) {
            case 'npm':
                return NpmShrinkwrapFile_1.NpmShrinkwrapFile.loadFromString(shrinkwrapContent);
            case 'pnpm':
                return PnpmShrinkwrapFile_1.PnpmShrinkwrapFile.loadFromString(shrinkwrapContent);
            case 'yarn':
                return YarnShrinkwrapFile_1.YarnShrinkwrapFile.loadFromString(shrinkwrapContent);
            default:
                throw new Error(`Invalid package manager: ${packageManager}`);
        }
    }
