...
const InputsSnapshot_1 = require("./incremental/InputsSnapshot");
/**
 * @beta
 */
class ProjectChangeAnalyzer {
    ...
    async getChangedProjectsAsync(options) {
        ...
        const changedFiles = (0, package_deps_hash_1.getRepoChanges)(repoRoot, mergeCommit, gitPath);
     ...
     * @internal
     */
    async _tryGetSnapshotProviderAsync(projectConfigurations, terminal) {
        var _a, _b;
        try {
            const gitPath = this._git.getGitPathOrThrow();
            ...
            const rootDirectory = (0, package_deps_hash_1.getRepoRoot)(rushConfiguration.rushJsonFolder, gitPath);
            ...
            if (rushConfiguration.packageManager === 'pnpm') {
                ...
                for (const project of rushConfiguration.projects) {
                    ...
                    const relativeProjectShrinkwrapFilePath = node_core_library_1.Path.convertToSlashes(path.relative(rootDirectory, projectShrinkwrapFilePath));
                    additionalRelativePathsToHash.push(relativeProjectShrinkwrapFilePath);
                }
                await node_core_library_1.Async.forEachAsync(absoluteFilePathsToCheck, async (filePath) => {
                    if (!rushConfiguration.subspacesFeatureEnabled && !(await node_core_library_1.FileSystem.existsAsync(filePath))) {
                ...
                const shrinkwrapFile = node_core_library_1.Path.convertToSlashes(path.relative(rootDirectory, rushConfiguration.defaultSubspace.getCommittedShrinkwrapFilePath(currentVariant)));
            ...
            return async function tryGetSnapshotAsync() {
                ...
                try {
                    const [hashes, additionalFiles] = await Promise.all([
                        (0, package_deps_hash_1.getRepoStateAsync)(rootDirectory, additionalRelativePathsToHash, gitPath),
                    ...
                    const additionalHashes = new Map(await (0, package_deps_hash_1.hashFilesAsync)(rootDirectory, additionalFiles, gitPath));
                    return new InputsSnapshot_1.InputsSnapshot({
                        additionalHashes,
...
async function getAdditionalFilesFromRushProjectConfigurationAsync(additionalGlobs, rootRelativeLookupByPath, rootDirectory, terminal) {
    ...
    await node_core_library_1.Async.forEachAsync(additionalGlobs, async (item) => {
        ...
        for (const match of matches) {
            ...
            const rootRelativeFilePath = node_core_library_1.Path.convertToSlashes(path.relative(rootDirectory, path.resolve(project.projectFolder, match)));
