...
class RushInstallManager extends BaseInstallManager_1.BaseInstallManager {
    constructor(rushConfiguration, rushGlobalFolder, purgeManager, options) {
        ...
        this._tempProjectHelper = new TempProjectHelper_1.TempProjectHelper(this.rushConfiguration, rushConfiguration.defaultSubspace);
    }
    /**
    ...
    async prepareCommonTempAsync(subspace, shrinkwrapFile) {
        ...
        console.log('\n' + terminal_1.Colorize.bold('Updating temp projects in ' + tempProjectsFolder));
        ...
        if (!shrinkwrapFile) {
            ...
            console.log();
            // eslint-disable-next-line no-console
            console.log(terminal_1.Colorize.red('The shrinkwrap file had previously been updated to support workspaces. Run "rush update --full" ' +
        ...
        for (const rushProject of sortedRushProjects) {
            ...
            if (shouldOverwrite) {
                try {
                    ...
                    // eslint-disable-next-line no-console
                    console.log(terminal_1.Colorize.yellow(error));
            ...
            if (packageJson.saveIfModified()) {
                // eslint-disable-next-line no-console
                console.log(terminal_1.Colorize.yellow(`"${rushProject.packageName}" depends on one or more local packages which used "workspace:" ` +
        ...
        InstallHelpers_1.InstallHelpers.generateCommonPackageJson(this.rushConfiguration, this.rushConfiguration.defaultSubspace, commonDependencies);
        stopwatch.stop();
        // eslint-disable-next-line no-console
    ...
    async installAsync(cleanInstall, subspace) {
        ...
        await Utilities_1.Utilities.executeCommandWithRetryAsync({
            ...
            if (this.rushConfiguration.packageManager === 'pnpm') {
                // eslint-disable-next-line no-console
                console.log(terminal_1.Colorize.yellow(`Deleting the "node_modules" folder`));
                this.installRecycler.moveFolder(commonNodeModulesFolder);
        ...
        if (this.rushConfiguration.packageManager === 'npm') {
            // eslint-disable-next-line no-console
            console.log('\n' + terminal_1.Colorize.bold('Running "npm shrinkwrap"...'));
    ...
    async postInstallAsync(subspace) {
        if (!this.options.noLink) {
            ...
            // eslint-disable-next-line no-console
            console.log('\n' + terminal_1.Colorize.yellow('Since "--no-link" was specified, you will need to run "rush link" manually.'));
        }
    }
    /**
    ...
    async _fixupNpm5RegressionAsync() {
        ...
        if (anyChanges) {
            // eslint-disable-next-line no-console
            console.log('\n' + terminal_1.Colorize.yellow(terminal_1.PrintUtilities.wrapWords(`Applied workaround for NPM 5 bug`)) + '\n');
        }
    }
    /**
    ...
    _findMissingTempProjects(shrinkwrapFile) {
        ...
        for (const rushProject of this.rushConfiguration.projects) {
            if (!tempProjectNames.has(rushProject.tempProjectName)) {
                // eslint-disable-next-line no-console
                console.log('\n' +
                    terminal_1.Colorize.yellow(terminal_1.PrintUtilities.wrapWords(`Your ${this.rushConfiguration.shrinkwrapFilePhrase} is missing the project "${rushProject.packageName}".`)) +
                    '\n');
