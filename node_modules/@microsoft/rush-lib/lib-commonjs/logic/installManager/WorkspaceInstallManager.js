...
class WorkspaceInstallManager extends BaseInstallManager_1.BaseInstallManager {
    ...
    async doInstallAsync() {
        ...
        if (this.options.noLink) {
            ...
            console.log(terminal_1.Colorize.red('The "--no-link" option was provided but is not supported when using workspaces. Run the command again ' +
    ...
    async prepareCommonTempAsync(subspace, shrinkwrapFile) {
        ...
        // eslint-disable-next-line no-console
        console.log('\n' + terminal_1.Colorize.bold('Updating workspace files in ' + subspace.getSubspaceTempFolderPath()));
        ...
        if (!shrinkwrapFile) {
            ...
            if (!shrinkwrapFile.isWorkspaceCompatible && !fullUpgrade) {
                ...
                console.log();
                // eslint-disable-next-line no-console
                console.log(terminal_1.Colorize.red('The shrinkwrap file has not been updated to support workspaces. Run "rush update --full" to update ' +
                    'the shrinkwrap file.'));
                throw new node_core_library_1.AlreadyReportedError();
            }
            ...
            if (orphanedProjects.length > 0) {
                for (const orphanedProject of orphanedProjects) {
                    shrinkwrapWarnings.push(`Your ${this.rushConfiguration.shrinkwrapFilePhrase} references "${orphanedProject}" ` +
                        `which was not found in ${RushConstants_1.RushConstants.rushJsonFilename}`);
                }
                shrinkwrapIsUpToDate = false;
            }
        }
        ...
        const workspaceFile = new PnpmWorkspaceFile_1.PnpmWorkspaceFile(path.join(subspace.getSubspaceTempFolderPath(), 'pnpm-workspace.yaml'));
        ...
        for (const rushProject of this.rushConfiguration.projects) {
            ...
            for (const { name, version, dependencyType } of [
                ...
                if ((dependencySpecifier.specifierType === DependencySpecifier_1.DependencySpecifierType.Version ||
                    ...
                    if (!semver.satisfies(referencedLocalProject.packageJsonEditor.version, dependencySpecifier.versionSpecifier)) {
                        ...
                        console.log();
                        // eslint-disable-next-line no-console
                        console.log(terminal_1.Colorize.red(`"${rushProject.packageName}" depends on package "${name}" (${version}) which exists ` +
                    ...
                    if (!allowShrinkwrapUpdates) {
                        ...
                        console.log();
                        // eslint-disable-next-line no-console
                        console.log(terminal_1.Colorize.red(`"${rushProject.packageName}" depends on package "${name}" (${version}) which exists within ` +
            ...
            if (packageJson.saveIfModified()) {
                // eslint-disable-next-line no-console
                console.log(terminal_1.Colorize.yellow(`"${rushProject.packageName}" depends on one or more workspace packages which did not use "workspace:" ` +
            ...
            if (dependencyMetaList.length !== 0) {
                ...
                const relativePathFromTempFolderToPackageFolder = node_core_library_1.Path.convertToSlashes(`${relativeFromTempFolderToRootFolder}/${rushProject.projectRelativeFolder}`);
        ...
        if ((shrinkwrapFile === null || shrinkwrapFile === void 0 ? void 0 : shrinkwrapFile.importers) !== undefined) {
            for (const [key, value] of shrinkwrapFile === null || shrinkwrapFile === void 0 ? void 0 : shrinkwrapFile.importers) {
                const projectRelativePath = node_core_library_1.Path.convertToSlashes(key);
        ...
        const dependenciesMetaAreEqual = (0, objectUtilities_1.objectsAreDeepEqual)(expectedDependenciesMetaByProjectRelativePath, lockfileDependenciesMetaByProjectRelativePath);
        ...
        const overridesAreEqual = (0, objectUtilities_1.objectsAreDeepEqual)((_a = this.rushConfiguration.pnpmOptions.globalOverrides) !== null && _a !== void 0 ? _a : {}, (shrinkwrapFile === null || shrinkwrapFile === void 0 ? void 0 : shrinkwrapFile.overrides) ? Object.fromEntries(shrinkwrapFile === null || shrinkwrapFile === void 0 ? void 0 : shrinkwrapFile.overrides) : {});
        ...
        InstallHelpers_1.InstallHelpers.generateCommonPackageJson(this.rushConfiguration, subspace, undefined);
    ...
    async canSkipInstallAsync(lastModifiedDate, subspace, variant) {
        ...
        // returns false.
        return Utilities_1.Utilities.isFileTimestampCurrentAsync(lastModifiedDate, potentiallyChangedFiles);
    }
    /**
    ...
    async installAsync(cleanInstall, subspace) {
        ...
        if (terminal_1.ConsoleTerminalProvider.supportsColor) {
            packageManagerEnv.FORCE_COLOR = '1';
        }
        const commonNodeModulesFolder = path.join(subspace.getSubspaceTempFolderPath(), RushConstants_1.RushConstants.nodeModulesFolderName);
        ...
        const doInstallInternalAsync = async (options) => {
            ...
            // eslint-disable-next-line no-console
            console.log('\n' +
                terminal_1.Colorize.bold(`Running "${this.rushConfiguration.packageManager} install" in` +
            ...
            if (this.options.debug ||
                ...
                // eslint-disable-next-line no-console
                console.log('\n' +
                    terminal_1.Colorize.green('Invoking package manager: ') +
            ...
            const pnpmTips = [];
            for (const [customTipId, customTip] of Object.entries(CustomTipsConfiguration_1.PNPM_CUSTOM_TIPS)) {
            ...
            const onPnpmStdoutChunk = pnpmTips.length > 0
                ...
                : undefined;
            try {
                await Utilities_1.Utilities.executeCommandWithRetryAsync({
        ...
        const projectNodeModulesFolders = [
            path.join(subspace.getSubspaceTempFolderPath(), RushConstants_1.RushConstants.nodeModulesFolderName),
    ...
    async postInstallAsync(subspace) {
        ...
        const tempShrinkwrapFile = ShrinkwrapFileFactory_1.ShrinkwrapFileFactory.getShrinkwrapFile(this.rushConfiguration.packageManager, this.rushConfiguration.pnpmOptions, subspace.getTempShrinkwrapFilename());
        if (tempShrinkwrapFile) {
            // Write or delete all project shrinkwraps related to the install
            await node_core_library_1.Async.forEachAsync(subspace.getProjects(), async (project) => {
                var _a;
        ...
        if (this.rushConfiguration.subspacesFeatureEnabled &&
            ...
            const modulesFilePath = `${tempNodeModulesPath}/${RushConstants_1.RushConstants.pnpmModulesFilename}`;
            if (subspace.subspaceName.startsWith('split_') &&
                subspace.getProjects().length === 1 &&
                (await node_core_library_1.FileSystem.existsAsync(modulesFilePath))) {
                // Find the .modules.yaml file in the subspace temp/node_modules folder
                const modulesContent = await node_core_library_1.FileSystem.readFileAsync(modulesFilePath);
                const yamlContent = js_yaml_1.default.load(modulesContent, { filename: modulesFilePath });
                ...
                for (const value of Object.values(hoistedDependencies)) {
                    for (const [filePath, type] of Object.entries(value)) {
                        if (type === 'public') {
                            if (Utilities_1.Utilities.existsOrIsSymlink(`${projectNodeModulesPath}/${filePath}`)) {
                                await node_core_library_1.FileSystem.deleteFolderAsync(`${projectNodeModulesPath}/${filePath}`);
                            }
                            // If we don't already have a symlink for this package, create one
                            const parentDir = Utilities_1.Utilities.trimAfterLastSlash(`${projectNodeModulesPath}/${filePath}`);
                            await node_core_library_1.FileSystem.ensureFolderAsync(parentDir);
                            BaseLinkManager_1.BaseLinkManager._createSymlink({
            ...
            for (const dependencyProject of subspaceDependencyProjects) {
                ...
                if (!Utilities_1.Utilities.existsOrIsSymlink(symlinkToCreate)) {
                    const parentFolder = Utilities_1.Utilities.trimAfterLastSlash(symlinkToCreate);
                    await node_core_library_1.FileSystem.ensureFolderAsync(parentFolder);
                    BaseLinkManager_1.BaseLinkManager._createSymlink({
        ...
        // TODO: Remove when "rush link" and "rush unlink" are deprecated
        await new FlagFile_1.FlagFile(subspace.getSubspaceTempFolderPath(), RushConstants_1.RushConstants.lastLinkFlagFilename, {}).createAsync();
    }
    /**
...
function createObjectChecksum(obj) {
    const s = JSON.stringify(node_core_library_1.Sort.sortKeys(obj));
