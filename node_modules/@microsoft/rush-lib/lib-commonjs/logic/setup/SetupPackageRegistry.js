...
class SetupPackageRegistry {
    constructor(options) {
        ...
        this._terminal = new terminal_1.Terminal(new terminal_1.ConsoleTerminalProvider({
    ...
    async checkAndSetupAsync() {
        ...
        this._terminal.writeLine();
        const packageRegistry = this._artifactoryConfiguration.configuration.packageRegistry;
        const fixThisProblem = await TerminalInput_1.TerminalInput.promptYesNoAsync({
        ...
        const hasArtifactoryAccount = await TerminalInput_1.TerminalInput.promptYesNoAsync({
        ...
        if (this._messages.visitWebsite) {
            ...
            if (artifactoryWebsiteUrl) {
                this._terminal.writeLine('  ', terminal_1.Colorize.cyan(artifactoryWebsiteUrl));
                this._terminal.writeLine();
            }
        }
        this._writeInstructionBlock(this._messages.locateUserName);
        let artifactoryUser = await TerminalInput_1.TerminalInput.promptLineAsync({
            message: this._messages.userNamePrompt
        });
        this._terminal.writeLine();
        artifactoryUser = artifactoryUser.trim();
        if (artifactoryUser.length === 0) {
            this._terminal.writeLine(terminal_1.Colorize.red('Operation aborted because the input was empty'));
            this._terminal.writeLine();
            throw new node_core_library_1.AlreadyReportedError();
        }
        this._writeInstructionBlock(this._messages.locateApiKey);
        let artifactoryKey = await TerminalInput_1.TerminalInput.promptPasswordLineAsync({
    ...
    _mergeLinesIntoNpmrc(npmrcPath, linesToAdd) {
        ...
        this._terminal.writeLine();
        this._terminal.writeLine(terminal_1.Colorize.green('Adding Artifactory token to: '), npmrcPath);
        const npmrcLines = [];
    ...
    static _tryFindJson(dirtyOutput) {
        const lines = node_core_library_1.Text.splitByNewLines(dirtyOutput);
        let startIndex;
        let endIndex;
