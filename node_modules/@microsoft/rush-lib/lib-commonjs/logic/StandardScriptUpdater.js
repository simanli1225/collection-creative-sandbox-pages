...
const _scripts = [
    {
        scriptName: PathConstants_1.installRunScriptFilename,
        headerLines: [
            ...
            '//',
            `//    node common/scripts/${PathConstants_1.installRunScriptFilename} qrcode@1.2.2 qrcode https://rushjs.io`
        ]
    },
    {
        scriptName: PathConstants_1.installRunRushScriptFilename,
        headerLines: [
            '// This script is intended for usage in an automated build environment where the Rush command may not have',
            '// been preinstalled, or may have an unpredictable version.  This script will automatically install the version of Rush',
            `// specified in the ${RushConstants_1.RushConstants.rushJsonFilename} configuration file (if not already installed), and then pass a command-line to it.`,
            '// An example usage would be:',
            '//',
            `//    node common/scripts/${PathConstants_1.installRunRushScriptFilename} install`
        ]
    },
    {
        scriptName: PathConstants_1.installRunRushxScriptFilename,
        headerLines: [
            '// This script is intended for usage in an automated build environment where the Rush command may not have',
            '// been preinstalled, or may have an unpredictable version.  This script will automatically install the version of Rush',
            `// specified in the ${RushConstants_1.RushConstants.rushJsonFilename} configuration file (if not already installed), and then pass a command-line to the`,
            '// rushx command.',
            '//',
            '// An example usage would be:',
            '//',
            `//    node common/scripts/${PathConstants_1.installRunRushxScriptFilename} custom-command`
        ]
    }
];
const _pnpmOnlyScripts = [
    {
        scriptName: PathConstants_1.installRunRushPnpmScriptFilename,
        headerLines: [
            '// This script is intended for usage in an automated build environment where the Rush command may not have',
            '// been preinstalled, or may have an unpredictable version.  This script will automatically install the version of Rush',
            `// specified in the ${RushConstants_1.RushConstants.rushJsonFilename} configuration file (if not already installed), and then pass a command-line to the`,
            ...
            '// An example usage would be:',
            '//',
            `//    node common/scripts/${PathConstants_1.installRunRushPnpmScriptFilename} pnpm-command`
        ]
    }
];
...
class StandardScriptUpdater {
    ...
    static async updateAsync(rushConfiguration) {
        await node_core_library_1.FileSystem.ensureFolderAsync(rushConfiguration.commonScriptsFolder);
        let anyChanges = false;
        await node_core_library_1.Async.forEachAsync(getScripts(rushConfiguration), async (script) => {
    ...
    static async _updateScriptOrThrowAsync(script, rushConfiguration, throwInsteadOfCopy) {
        ...
        let targetContent;
        try {
            targetContent = await node_core_library_1.FileSystem.readFileAsync(targetFilePath);
        }
        catch (e) {
        ...
        if (!filesAreSame) {
            if (throwInsteadOfCopy) {
                ...
                await node_core_library_1.FileSystem.writeFileAsync(targetFilePath, sourceNormalized);
            }
        }
        return !filesAreSame;
    }
    ...
    static async _getExpectedFileDataAsync({ scriptName, headerLines }) {
        const sourceFilePath = `${PathConstants_1.scriptsFolderPath}/${scriptName}`;
        let sourceContent = await node_core_library_1.FileSystem.readFileAsync(sourceFilePath);
