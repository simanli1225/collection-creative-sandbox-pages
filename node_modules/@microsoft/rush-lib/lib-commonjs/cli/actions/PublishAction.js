...
const PolicyValidator = __importStar(require("../../logic/policy/PolicyValidator"));
...
class PublishAction extends BaseRushAction_1.BaseRushAction {
    constructor(parser) {
        ...
        this._includeAll = this.defineFlagParameter({
            ...
            parameterShortName: undefined,
            description: `If this flag is specified, all packages with shouldPublish=true in ${RushConstants_1.RushConstants.rushJsonFilename} ` +
    ...
    async runAsync() {
        ...
        // eslint-disable-next-line no-console
        console.log('\n' + terminal_1.Colorize.green('Rush publish finished successfully.'));
    }
    /**
    ...
    async _publishChangesAsync(git, publishGit, allPackages) {
        ...
        if (changeManager.hasChanges()) {
            ...
            if (await git.hasUncommittedChangesAsync()) {
                ...
                await publishGit.commitAsync(this.rushConfiguration.gitVersionBumpCommitMessage || VersionAction_1.DEFAULT_PACKAGE_UPDATE_MESSAGE, !this._ignoreGitHooksParameter.value);
    ...
    async _npmPublishAsync(packageName, packagePath) {
        ...
        if (this.rushConfiguration.projectsByName.get(packageName).shouldPublish) {
            ...
            await PublishUtilities_1.PublishUtilities.execCommandAsync(!!this._publish.value, packageManagerToolFilename, args, packagePath, env, secretSubstring);
        }
    }
    async _packageExistsAsync(packageConfig) {
        ...
        const args = [];
        this._addSharedNpmConfig(env, args);
        const publishedVersions = await Npm_1.Npm.getPublishedVersionsAsync(packageConfig.packageName, packageConfig.publishFolder, env, args);
    ...
    async _npmPackAsync(packageName, project) {
        const args = ['pack'];
        const env = PublishUtilities_1.PublishUtilities.getEnvArgs();
        await PublishUtilities_1.PublishUtilities.execCommandAsync(!!this._publish.value, this.rushConfiguration.packageManagerToolFilename, args, project.publishFolder, env);
