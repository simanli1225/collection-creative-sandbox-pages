...
export class PnpmLinkManager extends BaseLinkManager {
    ...
    async createSymlinksForProjectsAsync(force) {
        ...
        if (useWorkspaces) {
            ...
            console.log(Colorize.red('Linking is not supported when using workspaces. Run "rush install" or "rush update" ' +
    ...
    async _linkProjectsAsync() {
        if (this._rushConfiguration.projects.length > 0) {
            ...
            const pnpmShrinkwrapFile = PnpmShrinkwrapFile.loadFromFile(this._rushConfiguration.defaultSubspace.getTempShrinkwrapFilename());
            if (!pnpmShrinkwrapFile) {
                throw new InternalError(`Cannot load shrinkwrap at "${this._rushConfiguration.defaultSubspace.getTempShrinkwrapFilename()}"`);
            }
